...
function MainWindow()
    ...
    function win.On.ExecuteButton.Clicked(ev)
        ...
        if execute(items["EnableDataLevelAdjustment"]["Checked"],
                items["DataLevelAdjustmentType"]["CurrentIndex"],
                items["EnableMetadataParser"]["Checked"]) then
...
function get_clips(folder, result)
...
function assign_data_level(clip, metadata, assign_type)
    if metadata and metadata["Camera Manufacturer"] == "Atomos" then
        ...
        if assign_type == 0 and (string.find(string.upper(gamma_notes), "LOG") or string.find(camera_notes, "Range: Legal")) or assign_type == 1 then
            if clip:SetClipProperty("Data Level", "Full") then
                print(string.format("Assign %s data level full successfully.", clip:GetName()))
            else
                print(string.format("Assign %s data level full failed.", clip:GetName()))
                return False
            end
        end
    end
...
function execute(assign_data_level_enabled, assign_type, parse_metadata_enabled)
    ...
    print("Start match input color space and apply custom grading rules.")
    ...
    for _, clip in ipairs(clips) do
        metadata = parse_metadata_enabled and parse_metadata(clip) or clip:GetMetadata()
        ...
        if is_rcm then
            ...
            if input_color_space ~= "" then
                if clip:GetClipProperty("Input Color Space") == input_color_space then
                    print(string.format("%s Input Color Space is already set", clip:GetName()))
                    goto continue
                end
                if clip:SetClipProperty("Input Color Space", input_color_space) then
                    print(string.format("%s Set Input Color Space %s Successfully.", clip:GetName(), input_color_space))
                else
                    print(string.format("%s Set Input Color Space %s Failed!", clip:GetName(), input_color_space))
                end
            else
                success = false
                print(string.format("%s Does Not Found Input Color Space Match Rule!", clip:GetName()))
            end
        end
        ::continue::
        if assign_data_level_enabled then
            if not assign_data_level(clip, metadata, assign_type) then
                success = false
            end
        end
    end
    if success then
        ...
        print("Some error happened, please check console details.")
        return false
    end
end
